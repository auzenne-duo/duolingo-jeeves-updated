...
class QualityReportProjectSection(QualityReportBase):
    def __init__(
        ...
        super().__init__(end_date, features, issues, project, start_date, title)
        if not self.issues:
            return
        self.score_history = score_history.copy()
        self.score_history.append((date_to_str(self.end_date), self.score_breakdown.overall_score))
