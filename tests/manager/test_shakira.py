    ...
    def test_get_slack_report_types(self):
        ...
        case.assertCountEqual(
            [
                ...
                {"name": "Content quality issue", "alsoPostsToJira": False},
...
def test_upload_artifacts_stream_seek_and_executor_order(monkeypatch):
    _, _, _, _, _, shakira_manager = _get_mocked_managers()
    # Call log to keep track of the order of calls
    call_log = []

    # Fake stream with logging
    class FakeStream:
        def seek(self, *args, **kwargs):
            call_log.append(("seek", args, kwargs))

        def read(self, *args, **kwargs):
            call_log.append(("read", args, kwargs))
            return b"fakeimage"

    fake_stream_screenshot = FakeStream()
    fake_stream_log = FakeStream()
    fake_file = MagicMock()
    fake_file.stream = fake_stream_screenshot
    fake_file.filename = "screenshot.png"
    fake_file_log = MagicMock()
    fake_file_log.stream = fake_stream_log
    fake_file_log.filename = "log.txt"
    files = {"screenshot": fake_file, "log.txt": fake_file_log}

    # Patch the executor and app_registry to capture submit calls
    class FakeExecutor:
        def submit(self, *args, **kwargs):
            call_log.append(("submit", args, kwargs))

    fake_executor = FakeExecutor()

    # Patch app_registry to return our fake executor
    monkeypatch.setattr("jeeves.manager.shakira.app_registry", lambda _: fake_executor)

    # Act
    shakira_manager.upload_artifacts("DLAA-123", files)

    # Assert each read has a seek before and after
    print(call_log)
    for i, c in enumerate(call_log):
        if c[0] == "read":
            assert (
                i < len(call_log) - 1 and call_log[i + 1][0] == "seek"
            ), f"Read at index {i} is not immediately followed by seek"

    # Assert
    # All 'submit' calls must come after all 'seek' and 'read' calls
    last_stream_call = max(i for i, c in enumerate(call_log) if c[0] in ("seek", "read"))
    submit_calls = [i for i, c in enumerate(call_log) if c[0] == "submit"]
    assert len(submit_calls) == 2, "Expected exactly two submit calls"
    assert all(
