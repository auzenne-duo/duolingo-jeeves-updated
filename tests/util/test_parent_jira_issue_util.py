...
class TestParentJiraIssueUtil(unittest.TestCase):
    def test_parse_parent_description(self):
        ...
        actual_output = parse_parent_description(body_text_in)
    ...
    def test_generate_parent_body_text_from_data(self):
        ...
        actual_output = generate_parent_body_text_from_data("AI summary", data_in)
        assert actual_output == desired_output

    def test_update_parent_data_from_child(self):
        parent_data = {
        ...
        actual_output = update_parent_data_from_child(parent_data, child_document)
        assert actual_output == desired_output

    def test_strip_parent_description(self):
        no_parent_metadata = "This is a description with no parent metadata."
        assert strip_parent_description(no_parent_metadata) == no_parent_metadata

        starting_description = "With metadata\nINTERFACE LANGUAGES:\nfr: 2\nNOT PRESENT: 1\nen: 1\n"
        assert strip_parent_description(starting_description) == "With metadata"

        trailing_description = (
            "INTERFACE LANGUAGES:\nfr: 2\nNOT PRESENT: 1\nen: 1\n\nTrailing description."
        )
        assert strip_parent_description(trailing_description) == "Trailing description."

        combined_description = "With metadata\nINTERFACE LANGUAGES:\nfr: 2\nNOT PRESENT: 1\nen: 1\n\nTrailing description."
        assert (
            strip_parent_description(combined_description) == "With metadata\nTrailing description."
        )

        long_description = """APP VERSIONS:
    5.92.3 (1569) play: 1
    5.93.3 (1573) play: 3
    ...
    OPERATING SYSTEMS:
    5.10.136-android12-9-25884040-abS901U1UEU2CWAI (S901U1UEU2CWAI): 4

    AREAS:
    """
        assert strip_parent_description(long_description) == "Middle description.\n"
